...
class VariationCache implements VariationCacheInterface {
  ...
  public function set(array $keys, $data, CacheableDependencyInterface $cacheability) {
    ...
    if (!array_key_exists($cid, $chain)) {
      $redirect = new CacheRedirect($cacheability);

      // We can easily find overly specific redirects by comparing their cache
      // contexts to the ones we have here. If a redirect has more contexts, it
      ...
      // If we couldn't find an overly specific step, we know that the last
      // cache ID in the chain is where the lookup failed. So we simply add a
      // redirect with the current cacheability there. Either way, $chain_cid
      // should now contain the cache ID we're interested in.
      //
      ...
      $this->cacheBackend->set($chain_cid, $redirect);
    }
